...
for i = 1:5;
    ...
    for j=1:nBlock
        str1 = strcat(['Gaussian/',num2str(nBlock),'/A', num2str(j), '.dat']);
        str2 = strcat(['Gaussian/',num2str(nBlock),'/xs', num2str(j), '.dat']);
        mmwrite(str1,A( :,(j-1)*n/nBlock+1: j*n/nBlock));
        mmwrite(str2,xs((j-1)*n/nBlock+1: j*n/nBlock,:));
    end
